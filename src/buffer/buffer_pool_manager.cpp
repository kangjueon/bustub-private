...
Page *BufferPoolManager::FetchPageImpl(page_id_t page_id) {
  ...
  latch_.lock();  //protecting Critical sections
  ...
  if (search != page_table_.end()){
    ...
    retPage->pin_count_++;
    replacer_ ->Pin(fid);
    latch_.unlock();   //protecting Critical sections
    return retPage;
  }  
  ...
  if(!replacer_->Victim(&pageFrameID)) {  //if free_list does not contain any free frame, then have replacer_find victim to evict
    latch_.unlock();
    return nullptr;
  }
  replPage = &pages_[pageFrameID];  
